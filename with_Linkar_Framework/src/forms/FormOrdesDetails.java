...
public class FormOrdesDetails extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public FormOrdesDetails(LinkarClient linkarClt) { //NEWFRAMEWORK: Replace LinkarClt for LinkarClient
		...
		txtSelect.setToolTipText("Insert Id or Empty");
					...
					{		
						...
						if (clkorders.size() > 0)
						{
							for (int i = 0; i < clkorders.size(); i++)
							{
								...
								fila[0] = clkorders.get(i).getId();
								((DefaultTableModel)table.getModel()).addRow(fila);
							}
							
							CLkOrder clkorder = new CLkOrder(linkarClt);
							clkorder.setId(clkorders.get(0).getId());
