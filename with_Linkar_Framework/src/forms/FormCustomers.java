...
public class FormCustomers extends JDialog {
	...
	public FormCustomers(LinkarClient linkarClt) { //NEWFRAMEWORK: Replace LinkarClt for LinkarClient
		setTitle("Customers");
		...
		contentPanel.setLayout(null);
		{
						...
						if (clkcustomers != null)
						{
							...
							{					
								for (int i = 0; i < clkcustomers.size(); i++)
								{
									...
									fila[0] = clkcustomers.get(i).getId();
									((DefaultTableModel)table.getModel()).addRow(fila);
								}
								
								clkcustomer = clkcustomers.get(0);
								txtId.setText(clkcustomer.getId());
				...
				public void actionPerformed(ActionEvent e) {
					...
					else
					{
						...
						for (int i = rowCount - 1; i >= 0; i--) {
							...
							if (id.equals(clkcustomer.getId()))
							{
								dm.removeRow(i);
								break;
							}				    
						...
						for (int i = 0; i < clkcustomers.size(); i++)
				        {
				        	...
				        	if (c.getId().equals(clkcustomer.getId()))
				        	{	  
			...
			txtSelectClause.setToolTipText("Insert Id or Name");
